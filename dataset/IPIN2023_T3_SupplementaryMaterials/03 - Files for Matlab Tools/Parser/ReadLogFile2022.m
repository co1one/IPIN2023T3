...
Ble4=ones(numlineas,5)*NaN; index_Ble4=1;
for i = 1:numlineas
Ble4UUID(i,:) = '00000000000000000000000000000000';
end
...
while (~eof)
    ...
    while (~linea_con_medida && ~eof)
        ...
        if ( idx_linea<=numlineas) % hay lineas por procesar
            ...
            % Temperature data:   	'TEMP;AppTimestamp(s);SensorTimestamp(s);temp(ºC);Accuracy(integer)'
            % Orientation data:   	'AHRS;AppTimestamp(s);SensorTimestamp(s);PitchX(º);RollY(º);YawZ(º);RotVecX();RotVecY();RotVecZ();Accuracy(int)'
            ...
            % IMU XSens data:     	'IMUX;AppTimestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(º);Pitch(º);Yaw(º);Pressure(mbar);Temp(ºC)'
            % IMU LPMS-B data:    	'IMUL;AppTimestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(º);Pitch(º);Yaw(º);Pressure(mbar);Temp(ºC)'
            ...
            if (numel(linea)>0 && ~strcmp(linea(1),'%') )
                ...
                if ( strfind(linea,'WIFI'))  % Es una linea de WIFI.
                    ...
                    % p.ej.: "WIFI;22.365;8051.234;portal-csic;00:0b:86:27:36:c1;2472;-71"
              ...
              if ( strfind(linea,'BLE4'))  % Limited to iBeacon in IPIN Competition 2022
              % NEW BLE 4.0 data:       	'BLE4;AppTimestamp(s);iBeacon;MAC;RSSI(dBm);Power;MajorID;MinorID;UUID'
                    cell_array=textscan(linea,'%*s %f %*s %*s %f %*s %f %f %s','delimiter',';');
                    datos(1)=cell_array{1,1}; % timestamp
                    datos(2)=cell_array{1,3}; % Major
                    datos(3)=cell_array{1,4}; % Minor
                    UUID=strrep(cell2mat(cell_array{1,5}),'-',''); % UUID
                    datos(4)=cell_array{1,2}; % RSSI
                    tipo='BLE4';
                    % disp(['Linea: ',linea]);
                end
                if ( strfind(linea,'IMUL'))  % Es una linea de IMUL. p.ej.: "IMUL;0.929;1907.2437;11782;0.22992;-0.15328;9.31184;-0.01853;0.01550;0.00004;-24.02985;56.41791;22.50000;-1.28843;1.03222;121.61204;936.870;0.00"
                    % 21 campos: Timestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(�);Pitch(�);Yaw(�);Quat1;Quat2;Quat3;Quat4;Pressure(mbar);Temp(�C)'");
	...
	if strcmp(tipo,'BLE4')
        Ble4(index_Ble4,1:4)  =datos;
        Ble4UUID(index_Ble4,:) = UUID;
...
if ~isempty(Ble4)
    figure(idx_fig); idx_fig=idx_fig+1;
    plot(Ble4(:,1),Ble4(:,4),'r.'); hold on;
    xlabel('time(s)'); ylabel('RSS (dBm)'); hold off;
    title(['Ble4, Freq.: ',num2str(size(Ble4,1)/Ble4(end,1)),' Hz'])
    legend({'Ble4'});
end
...
if ~isempty(Gnss)
    figure(idx_fig); idx_fig=idx_fig+1;
    subplot(4,2,1);
    plot(Gnss(:,1),Gnss(:,3),'r-');ylabel('Lat(�)');
    title(['GNSS data - Freq: ',num2str(size(Gnss,1)/Gnss(end,1)),' Hz']);
    subplot(4,2,2);
    plot(Gnss(:,1),Gnss(:,4),'r-'); ylabel('Long(�)');
    title(['Resolution=[1e-4�, 1.1 m, 0.3 m/s] ']);
    subplot(4,2,3);
    plot(Gnss(:,1),Gnss(:,5),'r-'); ylabel('Height(m)');
    subplot(4,2,4);
    plot(Gnss(:,1),Gnss(:,7),'r-'); ylabel('Accuracy(m)');
    subplot(4,2,5);
    plot(Gnss(:,1),Gnss(:,6),'r-'); ylabel('Bearing(�)');
    subplot(4,2,6);
...
if ~isempty(Imul)  % -----------Comienzan los datos de IMUL--------------
    figure(idx_fig); idx_fig=idx_fig+1;
    % 21 campos: Timestamp(s);LPMSTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(�);Pitch(�);Yaw(�);Pressure(mbar);Temp(�C)'");
    ...
    xlabel('time(s)'); ylabel('Orientation (�)');
    ...
    subplot(2,1,2);
    plot(tiempos,Imul(:,21),'r-'); ylabel('Temperature (�C)');
    xlabel('time(s)');
...
if ~isempty(Imux)  % -----------Comienzan los datos de IMUX--------------
    figure(idx_fig); idx_fig=idx_fig+1;
    % 21 campos: Timestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(�);Pitch(�);Yaw(�);Pressure(mbar);Temp(�C)'");
    % indices de Imul: Acc en(4:6), Gyr en(7:9), Mag en(10:12), Euler en(13:15), Quat(16:19),
    %                  Pres en 20 y Temp en 21
